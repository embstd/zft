...
public class ThreadGetCase extends Thread{
	...
	public void showProcess(Context context,Handler mHandler,int flag, String timePeriod){
	...
	public void run(){
		...
		if(result!=null){
			...
			ArrayList<Object> list = Assistant.parseXml(xml, Case.class);
...
//			int i=0;
			...
			for(Object obj:list){
				...
				Case item = (Case)obj;
				Config.currentCaseList.add(item);
//				print.out("i="+i);
...
//				mHandler.sendMessage(mHandler.obtainMessage(Msg.LISTVIEW_SHOW));
				mHandler.sendMessage(mHandler.obtainMessage(Msg.GETCASELIST));
			}else if(flag==1){
				//用户刷新,不需要再显示list
...
//			if(Config.test==1){
				result = Server.getCase(Config.user.getUserName(),timePeriod);
//			}else{
//				result = test();
//				Config.test=1;
//			}
