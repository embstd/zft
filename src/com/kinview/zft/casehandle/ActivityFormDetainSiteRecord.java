...
public class ActivityFormDetainSiteRecord extends Activity {
...
//		position = intent.getIntExtra("position", 0);
		...
		reportCase.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormDetainSiteRecord.this, myHandler, Msg.PROMPT_PRINT,
		...
		pickDate.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (pickDate.equals((Button)v)){
					msg.what = ActivityFormDetainSiteRecord.SHOW_DATAPICK;
				}
				ActivityFormDetainSiteRecord.this.dateHandler.sendMessage(msg);
			}
		});
...
//				.append("-").append((mDay < 10) ? "0" + mDay : mDay));
	...
	Handler dateHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case ActivityFormDetainSiteRecord.SHOW_DATAPICK:
...
//				refresh();
			...
			case Msg.ERROR_SERVER_CONNECT:
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormDetainSiteRecord.this,
						null, Msg.PROGRAM_EXIT, "连接", "服务器连接失败,请稍后重试", 0, "");
				break;
			case Msg.PROGRAM_EXIT:
				ThreadExit thread = new ThreadExit(ActivityFormDetainSiteRecord.this);
				thread.showProcess();
				break;
			...
			case Msg.TITLE_PROGRESS_STOP:
				...
				break;
			case Msg.ONBACK:
				it = new Intent(ActivityFormDetainSiteRecord.this,ActivityFormList.class);
				b = new Bundle ();
			...
			case Msg.REQUEST_SUCCESS:
				Dialog.showDialog(Dialog.OK, ActivityFormDetainSiteRecord.this,
...
//		et_evidence_registration_10 = (EditText)this.findViewById(R.id.evidence_registration_10_txt);
		...
		tv_form_title_4.setText(getResources().getString(R.string.form_title_17_name));
	}
...
//		.append(tempCase.getLsh()).append("_");
		...
		.append("            。")
		.append("在出示执法证件， 表明身份并告知回避权后，对当事人进行调查询问，当事人陈述其经营行为未办理营业 执照，且此前城管执法人员曾要求其停止无照经营行为。 \r\n")
		
		.append("    执法人员当场告知当事人其行为已违反《无照经营查处取缔办法》第四条第一项涉 "
				+"嫌无照经营，并拟根据《无照经营查处取缔办法》第九条第五项的规定对其 ").append("                       ")
		.append("进行扣押，其有权依据《行政强制法》第八条、第十八条第五项的规定进行陈述和申辩。\r\n")
		
		.append("    当事人陈述和申辩：").append(et_evidence_registration_5.getText().toString().trim()).append("\r\n")
		.append("    在听取当事人陈述、申辩后，执法人员当场交付了扣押决定书,对当事人的 ").append("              ")
		.append("（详见扣押清单）实施了扣押。\r\n ")
...
//		return Html.fromHtml(content.toString()).toString();
	...
	private void posPrintProcess() {
		if (checkInputLegal()) {
			Intent it  = new Intent(ActivityFormDetainSiteRecord.this, ActivityPosPrint.class);
