...
public class ActivityFormSiteRecordInfo extends Activity {
...
//		caseId = intent.getIntExtra("caseId", 0);
		...
		reportCase.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormSiteRecordInfo.this, myHandler, Msg.PROMPT_PRINT,
...
//				refresh();
			...
			case Msg.ERROR_SERVER_CONNECT:
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormSiteRecordInfo.this,
						null, Msg.PROGRAM_EXIT, "连接", "服务器连接失败,请稍后重试", 0, "");
				break;
			case Msg.PROGRAM_EXIT:
				ThreadExit thread = new ThreadExit(ActivityFormSiteRecordInfo.this);
				thread.showProcess();
				break;
			...
			case Msg.TITLE_PROGRESS_STOP:
				...
				break;
			case Msg.ONBACK:
				it = new Intent(ActivityFormSiteRecordInfo.this,ActivityFormList.class);
				b = new Bundle ();
			...
			case Msg.REQUEST_SUCCESS:
				Dialog.showDialog(Dialog.OK, ActivityFormSiteRecordInfo.this,
...
//		return Html.fromHtml(content.toString()).toString();
	...
	private String getFormContentWhenPause(){
		...
		.append(et_citizen_phone.getText().toString().trim().replaceAll("_", "")).append("_");
...
//		return Html.fromHtml(content.toString()).toString();
	...
	private void posPrintProcess() {
		if (checkInputLegal()) {
			Intent it  = new Intent(ActivityFormSiteRecordInfo.this, ActivityPosPrint.class);
