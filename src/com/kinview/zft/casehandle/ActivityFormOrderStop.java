...
public class ActivityFormOrderStop extends Activity {
...
//	public int position = -1;
	...
	private TextView tv_order_stop_1;
	private TextView tv_order_stop_2;
	private TextView tv_order_stop_3;
	private TextView tv_order_stop_4;
	private TextView tv_order_stop_5;
	private TextView tv_order_stop_6;
	
	private EditText et_order_stop_1;
	private EditText et_order_stop_2;
	private EditText et_order_stop_3;
	private EditText et_order_stop_4;
	private EditText et_order_stop_5;
	private EditText et_order_stop_6;
...
//		position = intent.getIntExtra("position", 0);
		...
		setContentView(R.layout.form_order_stop);
		init();
		...
		reportCase.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormOrderStop.this, myHandler, Msg.PROMPT_PRINT,
...
//				Toast.makeText(ActivityForm.this, "hhhhhh", Toast.LENGTH_SHORT);
	...
	private void setEditTextLock(){
		lockEditText(et_order_stop_1);
		lockEditText(et_order_stop_2);
		lockEditText(et_order_stop_3);
		lockEditText(et_order_stop_4);
		lockEditText(et_order_stop_5);
		lockEditText(et_order_stop_6);
	...
	private void showCaseInfo(){
		...
		if (Config.currentNewCaseList.get(0).getOrganise().equals("") ||
			...
			et_order_stop_1.setText(Config.currentNewCaseList.get(0).getPerson());
		}else {
			et_order_stop_1.setText(Config.currentNewCaseList.get(0).getOrganise());
		}
//		et_order_stop_1.setText(Config.currentNewCaseList.get(0).getOrganise());
		et_order_stop_2.setText(Config.currentNewCaseList.get(0).getAddress());
//		et_order_stop_3.setText(Config.currentNewCaseList.get(0).getAddress());//违法建设
//		xiandingTimeStr = Config.getXiandingTimeString(Config.currentNewCaseList.get(0).getThetime());
		et_order_stop_4.setText("3");//限定日期
		et_order_stop_5.setText(Config.name);//联系人
		et_order_stop_6.setText(Config.user.getBranch_phone());//(部门)电话
	}
...
//				refresh();
			...
			case Msg.ERROR_SERVER_CONNECT:
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormOrderStop.this,
						null, Msg.PROGRAM_EXIT, "连接", "服务器连接失败,请稍后重试", 0, "");
				break;
			case Msg.PROGRAM_EXIT:
				ThreadExit thread = new ThreadExit(ActivityFormOrderStop.this);
				thread.showProcess();
				break;
			...
			case Msg.TITLE_PROGRESS_STOP:
				...
				break;
			case Msg.ONBACK:
				it = new Intent(ActivityFormOrderStop.this,ActivityFormList.class);
				b = new Bundle ();
			...
			case Msg.REQUEST_SUCCESS:
				Dialog.showDialog(Dialog.OK, ActivityFormOrderStop.this,
	...
	private void init(){
		tv_form_title_all_5 = (TextView)this.findViewById(R.id.form_title_all_7);
		tv_form_title_5 = (TextView)this.findViewById(R.id.form_title_7);
		tv_order_stop_1 = (TextView)this.findViewById(R.id.order_stop_1_name);
		tv_order_stop_2 = (TextView)this.findViewById(R.id.order_stop_2_name);
		tv_order_stop_3 = (TextView)this.findViewById(R.id.order_stop_3_name);
		tv_order_stop_4 = (TextView)this.findViewById(R.id.order_stop_4_name);
		tv_order_stop_5 = (TextView)this.findViewById(R.id.order_stop_5_name);
		tv_order_stop_6 = (TextView)this.findViewById(R.id.order_stop_6_name);
		
		et_order_stop_1 = (EditText)this.findViewById(R.id.order_stop_1_txt);
		et_order_stop_2 = (EditText)this.findViewById(R.id.order_stop_2_txt);
		et_order_stop_3 = (EditText)this.findViewById(R.id.order_stop_3_txt);
		et_order_stop_4 = (EditText)this.findViewById(R.id.order_stop_4_txt);
		et_order_stop_5 = (EditText)this.findViewById(R.id.order_stop_5_txt);
		et_order_stop_6 = (EditText)this.findViewById(R.id.order_stop_6_txt);
	...
	private String getFormContent(){
		...
		content.append(Config.posFormTitle +"执责停字[").append(String.valueOf(year)).append("]  \r\nNo.")
		.append(tempCase.getLsh()).append("_");
		content.append(et_order_stop_1.getText().toString().trim()).append(":\r\n\r\n");
		content.append("    你（单位）在 ").append(et_order_stop_2.getText().toString().trim())
		.append(" ，建设的").append(et_order_stop_3.getText().toString().trim())
		.append("  ，经检查，现场未能提供合法手续，已涉嫌违反《中华人民共和国城乡规划法》的有 关规定。现责令你（单位）立即停止上述行为，并在收到本通知书之日起 ")
		.append(et_order_stop_4.getText().toString().trim())
		.append("  日内，向  ");
//		.append(Config.user.getBranch_name())
...
//		content.append(Config.cgUnitName + Config.cgUnitNameDaDui)
		content.append("  提供上述行为的相关合法 手续，逾期不提供的，视作无合法手续，我局将依法查处。\r\n\r\n");
...
//		}
		...
		content
		.append(et_order_stop_1.getText().toString().trim()).append("_")
		.append(et_order_stop_2.getText().toString().trim()).append("_")
		.append(et_order_stop_3.getText().toString().trim()).append("_")
		.append(et_order_stop_4.getText().toString().trim()).append("_");
//		.append(Config.user.getBranch_name())
		if (Config.cityTag.equals("cztnq") || Config.cityTag.equals("czxbq") || Config.cityTag.equals("czwjq")
				|| Config.cityTag.equals("czzlq") || Config.cityTag.equals("czqsyq")){
				content.append(Config.cgUnitNameZhiDui);//部门
...
//		content.append(Config.cgUnitName + Config.cgUnitNameDaDui)
		content.append("_")
		.append(et_order_stop_5.getText().toString().trim()).append("_")
		.append(et_order_stop_6.getText().toString().trim()).append("_");
////		if (!et_order_stop_7.getText().toString().trim().equals("")){
//			content.append(et_order_stop_7.getText().toString().trim()).append("_");
////		}
////		if (!et_order_stop_8.getText().toString().trim().equals("")){
//			content.append(et_order_stop_8.getText().toString().trim()).append("_");
////		}
////		if (!et_order_stop_9.getText().toString().trim().equals("")) {
//			content.append(et_order_stop_9.getText().toString().trim()).append("_");
////		}
////		if (!et_order_stop_10.getText().toString().trim().equals("")) {
//			content.append(et_order_stop_10.getText().toString().trim()).append("_");
////		}
			content.append(year).append("_");	
			content.append(month).append("_");
			content.append(day);
//		.append(et_order_stop_7.getText().toString().trim()).append("_")
//		.append(et_order_stop_8.getText().toString().trim()).append("_")
//		.append(et_order_stop_9.getText().toString().trim()).append("_")
//		.append(et_order_stop_10.getText().toString().trim());
		return content.toString();
//		return Html.fromHtml(content.toString()).toString();
	}
	...
	private void posPrintProcess() {
		if (checkInputLegal()) {
			Intent it  = new Intent(ActivityFormOrderStop.this, ActivityPosPrint.class);
...
//		if (et_order_stop_1.getText().toString().trim().equals("")){
