...
public class ActivityFormDetainDeterminReg extends Activity {
...
//		position = intent.getIntExtra("position", 0);
		...
		reportCase.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormDetainDeterminReg.this, myHandler, Msg.PROMPT_PRINT,
		...
		pickDate.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (pickDate.equals((Button)v)){
					msg.what = ActivityFormDetainDeterminReg.SHOW_DATAPICK;
				}
				ActivityFormDetainDeterminReg.this.dateHandler.sendMessage(msg);
			}
		});
...
//		setTimeOfDay(); 
	...
	Handler dateHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case ActivityFormDetainDeterminReg.SHOW_DATAPICK:
...
//				refresh();
			...
			case Msg.ERROR_SERVER_CONNECT:
				Dialog.showDialog(Dialog.OKCANCEL, ActivityFormDetainDeterminReg.this,
						null, Msg.PROGRAM_EXIT, "连接", "服务器连接失败,请稍后重试", 0, "");
				break;
			case Msg.PROGRAM_EXIT:
				ThreadExit thread = new ThreadExit(ActivityFormDetainDeterminReg.this);
				thread.showProcess();
				break;
			...
			case Msg.TITLE_PROGRESS_STOP:
				...
				break;
			case Msg.ONBACK:
				it = new Intent(ActivityFormDetainDeterminReg.this,ActivityFormList.class);
				b = new Bundle ();
			...
			case Msg.REQUEST_SUCCESS:
				Dialog.showDialog(Dialog.OK, ActivityFormDetainDeterminReg.this,
...
//				setResult(0, it);
	...
	private void init(){
		...
		tv_form_title_4.setText(getResources().getString(R.string.form_title_16_name));
	}
...
//		content.append(tempform.getFormName()).append("_");
		content.append(Config.posFormTitle +"执扣字[").append(String.valueOf(year1)).append("]  \r\nNo.")
		.append(tempCase.getLsh()).append("_");
		
		
		content.append("当事人：").append("              \r\n").append("电话： ").append("              \r\n")
		.append("负责人：").append("              \r\n").append("职务：").append("              \r\n")
		.append("地址（住所）：").append("              \r\n\r\n")
		.append("    你(单位)因").append(et_evidence_registration_2.getText().toString().trim())
		.append("在").append(et_evidence_registration_3.getText().toString().trim())
		.append("进行无照经营的行为，涉嫌违反了国务院《无照经营查 第四条第一项处取缔办法》的规定。依据国务院《无照经营查处取缔办法》第九 条第五项的规定，本机关决定对你（单位）涉案 的")
		.append("             ").append("予以扣押 (详见《扣押清单》)，依据 ")
		.append("                    ").append("的规定， 扣押 日。上述物品，未经本机关同意，不得隐匿、销毁或转移。\r\n ")
		.append("   你(单位)应当在").append(et_evidence_registration_5.getText().toString().trim());
		if (Config.cityTag.equals("czjts")){
			content.append("金坛市城市管理行政执法局接受处理(地址:北环西路155号，电话：82805921)");
			content.append("接受处理。逾期不来接受处理，造成被 扣押物品自然损毁，由你（单位）自行承担。 \r\n")
			.append("    如你（单位）不服本决定，可以在接到本决定书之日起六十日内向 金坛市人 民政府或常州市城市管理行政执法局申请行政复议；也可以在三个月内直接向金 坛市人民法院起诉。 ");
		}
		content.append("保存地点：").append("                      \r\n");
...
//		return Html.fromHtml(content.toString()).toString();
	...
	private void posPrintProcess() {
		if (checkInputLegal()) {
			Intent it  = new Intent(ActivityFormDetainDeterminReg.this, ActivityPosPrint.class);
